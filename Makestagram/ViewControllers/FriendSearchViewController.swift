...
class FriendSearchViewController: UIViewController {
    ...
    var users: [PFUser]? //This is a local cache. It stores all the users this user is following.It is used to update the UI immediately upon user interaction, instead ofhaving to wait for a server response.
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        ParseHelper.getFollowingUsersForUser(PFUser.currentUser()!) { (results: [PFObject]?, error: NSError?) -> Void in
            ...
            self.followingUsers = relations.map {
                $0.objectForKey(ParseHelper.ParseFollowToUser) as! PFUser //What does $0 do 
            }
